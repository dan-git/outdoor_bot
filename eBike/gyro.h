  ...
  public:
    GyroStabilizer()
      ...
      kpGyro_(KP_GYRO), kiGyro_(KI_GYRO), kdGyro_(KD_GYRO), yawTotal_(0.0)
      {
        ...
        pidGyro_ = new pidControl(KP_GYRO, KI_GYRO, KD_GYRO, 10.0f, MAX_ANGULAR_ROBOT_VELOCITY);  // limit change to 10 deg/sec per cycle
    ...
    double readGyro()
    {
      ...
      if (abs(yawVel) < 0.5) gyroBaseline_ +=  (gyro_.g.z - gyroBaseline_) / 100.; // if we are just seeing noise, update the baseline
    ...
    virtual void stabilize(double *pidResult, double *angV, double commandedAngV, long deltaTime, double speedVelocityRatio, boolean startUpMode)
    {
      ...
      */ 
      ...
      if (DEBUG)
        {       
          DEBUG_SERIAL_PORT.print("current angular velocity in degs/sec = " );
