...
#define SHORT_MOVE_TO_TARGET 2.0
#define RADIAL_MOVE_TO_TARGET 3.0
...
int distanceToFirstTarget_, distanceToFirstTargetStaging_, radialMovesToTarget_;
...
bool radarGoodData_, radarGoodOrientation_, radarGoodLocation_, radarNewData_, usingRadar_, pastStagingPoint_, platforming_, atPlatform_;
...
bool phase1_, pauseNewCommand_, pauseCommanded_, previousState_, recoverTurnedAlready_, firstMoveToFirstTarget_, turnedToFaceFirstTarget_, firstMoveToTarget_, finalMoveToTarget_;
...
int HeadForHomeState_, MoveOntoPlatformState_, UserCommandState_, RadarApproachState_;
...
int platformNumber_;
int distanceToSwitchFromDirantToRadarDistance_ = SWITCH_FROM_DIR_ANT_TO_RADAR_DISTANCE;
...
bool getUserInput()
{
   ...
   phase1_ = true;
   firstMoveDistance_ = 2000;
   cout << "first move distance = " << firstMoveDistance_ << " mm" << endl;
   firstTurnAngle_ = 0;
	cout << "first turn angle = " << firstTurnAngle_ << endl;
   ...
   if (myStream2 >> compNumber)
   {
	...
	cout << "firstMove distance = " << firstMoveDistance_ << endl;
	
	
   cout << "Please enter the first turn angle: " << endl;
   getline(cin, input);
...
void radarCallback(const outdoor_bot::radar_msg::ConstPtr& msg)
{
	...
	if (!radarGoodLocation_) return; // only two radars reporting, so we only have distance

	angleToHomeRadar_ = msg->angleToHome;
	
	if (!radarGoodOrientation_) return; // three radars reporting, we have distance and angle to home
...
void on_enter_BootupState()
{
   ...
   turnedToFaceFirstTarget_ = false;
...
int on_update_CheckLinedUpState()
{
   ...
   */
   ...
   cout << "Do you want to move on to autonomous ops or retry BootUp?" << endl;
   if (askUserForGo())
   {  
     /*	
     if (phase1_)
   ...
   if (firstMoveToFirstTarget_)
   {
   	...
   	if (radarGoodData_ && distanceToHomeRadar_ > 4. && radarNewData_)
   	{
		...
		if (fabs(yaw_ - firstTurnAngle_) > 3.0 && alreadyTurned_ < 1)
		{
			msg.command = "autoMove";
			msg.distance = 0.;
			int negativeNetYaw = firstTurnAngle_- yaw_; // go in the opposite direction of the net yaw
		   msg.speed = 20. * sgn(negativeNetYaw); 
		   //msg.angle = (yaw_ - firstTurnAngle_) + (sgn(yaw_) * 2);  // we turn a little extra to compensate for having moved forward with yaw drift.
		   msg.angle = (negativeNetYaw) + (sgn(negativeNetYaw) * 2);
		...
		if (distanceToHome >= distanceToFirstTargetStaging_) 
		{
			...
			cout << "distance to home, distanceToFirstTargetStaging = " << distanceToHome << ", " << distanceToFirstTargetStaging_ << endl;
	   	cout << "approxRangeToTarget_ = " << approxRangeToTarget_ << endl;
	   	ROS_INFO("move completed in MoveToFirstTargetState, now doing alignment turn to face the target");
	   	msg.command = "autoMove";
			msg.distance = 0.;
		   msg.speed = 20.; 
		   msg.angle = (90. + firstTurnAngle_) - yaw_;			
			cout << "turning to face target = " << msg.angle << " degrees" << endl;
			turning_ = true;
			movement_pub_.publish(msg);
			movementComplete_ = false;
			firstMoveToFirstTarget_ = false;
   	...
   	if (distanceToHome >= 0)
   	{
   		...
   		else if (distanceToHome < distanceToFirstTargetStaging_ - INCREMENTAL_MOVE_TO_TARGET)
   		{
   			...
   			cout << "making incremental move to target, distanceToFirstTargetStaging, approxRangeToTarget = " << distanceToFirstTargetStaging_ << ", " << approxRangeToTarget_ << endl;
   			cout << "odom and radar distances to home = " << odomDistanceToHome_ << ", " << distanceToHomeRadar_ << endl;
   			cout << " we are using " << distanceToHome << " as our distance to home" << endl;
   			cout << " with a distance to FirstTarget = " << distanceToFirstTarget_ << endl;
   		}
   	...
   	msg.speed = 1000.;
		...
		movementComplete_ = false;
		return MoveToFirstTargetState_;
   }	// ends section of firstMoveToFirstTarget
...
//     return AvoidObstacleState_; // uncomment for real ops ************************
   ...
   if (centerX_ < 0 && (!finalMoveToTarget_)) // we don't have a target in view yet, so we will just turn or move forward a bit
	{    
		if (radialMovesToTarget_ > 4)
		{
			msg.command = "autoMove";
			msg.angle = 0.;
			msg.speed = 1000.;
			msg.distance = RADIAL_MOVE_TO_TARGET * 1000.;
			cout << "starting blind move, distance = " << msg.distance << " mm" << endl;
			moving_ = true;
			movement_pub_.publish(msg);
			movementComplete_ = false;
			radialMovesToTarget_++;
			return MoveToTargetState_;
		}
		msg.command = "autoMove";
		msg.angle = INCREMENTAL_TURN_TO_FIND_TARGET;
    ...
    case obn::DirAntFollower::Output::MOVE_FORWARD:
      {
        ...
        {
          // We're done!
          return RadarApproachState_;
        }
        // How far should we move forward?
        double distance_to_move = output.distance();
        if (distanceToHomeRadar_ - output.distance() < distanceToSwitchFromDirantToRadarDistance_)
        {
          // Don't overshoot where we want the cameras to take over.
          distance_to_move = distanceToHomeRadar_ - distanceToSwitchFromDirantToRadarDistance_;
...
void on_enter_RadarApproachState()
{
	/*outdoor_bot::movement_msg msg;
...
int on_update_RadarApproachState()
{
...
int on_update_HeadForHomeState()
{
 ...
 // static bool radarTurnedAlready = false;
  ...
  if (pauseCommanded_)	// we go here as the very first step in autonomous ops
   ...
   if ((!movementComplete_)  && (moving_ || turning_ || orienting_ || platforming_ || parking_ ))
   {
   	ros::spinOnce();
   	return HeadForHomeState_;
   }
   ...
   else
   {
   	movementComplete_ = false;
		...
		// so we wait a while between moves.  it takes 1 second to get brand new radar data
		...
		*/
    	...
    	else if (orienting_)
    	{
    		...
    		if (radarGoodLocation_) 
    		{
    			cout << "angle to home = " << angleToHomeRadar_ << endl;
    			if (radarGoodOrientation_) cout << "orientation to home = " << orientationToHomeRadar_ << endl;
    			else cout << "but not getting radar orientations right now" << endl;
    		}
   ...
   // now move or turn	
	...
	if (radarGoodLocation_)  
   {
   	if ((!turning_) && (alreadyTurned_ < 1) && (!pastStagingPoint_))
   	{
		   // send turn command to center the target		
			...
			if (fabs(msg.angle) < 5.  || (!radarGoodLocation_) || distanceToRadarStagingPoint_ < 2.) // no need to turn
			{
			   ...
			   turning_ = true;
			   movementComplete_ = true;
			   if (radarGoodLocation_) cout << " turn toward home was small enough to skip, it was = " << msg.angle << endl;
   	...
   	else if ((!moving_) && (!pastStagingPoint_))
   	{
			...
			{
				if (radarGoodOrientation_) msg.distance = (distanceToRadarStagingPoint_ * 1000.)/ 2.;
		...
		else if ((!orienting_) && (!alreadyOriented_) && !parking_)
		// we are now close and have good radar data, so time to orient to the platform
		{
         ...
         else
         {
		      ...
		      alreadyOriented_ = true;
				pastStagingPoint_ = true;
		      movementComplete_ = false;
				cout << "using radar to orient to home platform, distance, turning "<< distanceToHomeRadar_ << " meters, " << angleToHomeRadar_;
				cout << " degrees" << endl;
				return HeadForHomeState_;
			}
		}
	...
	} 	// end of section if radarGoodData
   ...
   */	
	...
	{
		cout << "call to radar failed, using optical range" << endl;    
		range_ = homeCameraRange_; // use optical range if radar fails
	}
	else if (nearestObstacle_ > 0.)
	{
		cout << "call to radar failed, no range from cameras, using laser range" << endl; 
		range_ = nearestObstacle_;
	}
	else 
   {
   	...
   	if (centerX_ > 0 || radarGoodData_)
		{
		  ...
		  else
		  {
			...
			//distanceToHomeRadarBeforeMove = distanceToHomeRadar_;	
			movement_pub_.publish(msg); 
		   ROS_INFO("turning -60 degrees at at range > 5 meters");
		   readSensors();
		   cout << "with a range_ of > 5 meters, laser sees the platform is at a distance = " << nearestObstacle_ << endl;
		  }		  	
   ...
   { 
		...
		if (range_ > PLATFORM_DISTANCE && (!atPlatform_))
		{
			cout << " going to the front of the platform  = " << (range_ - PLATFORM_DISTANCE) * 1000. << " mm" << endl;
			readSensors();
			cout << "laser sees the platform barrier at a range = " << nearestObstacle_ << endl;
			
			if (range_ - PLATFORM_DISTANCE < 1.0)
			{
				atPlatform_ = true;
				msg.distance = (range_ - PLATFORM_DISTANCE) * 1000.;
			}
			else msg.distance = 1000.;
			moving_ = false;
			platforming_ = true;
		...
		else if (range_ > PARKING_DISTANCE)
		{
			cout << " we are moving onto the platform = " << (range_ + 1.0 - PARKING_DISTANCE) * 1000. << " mm" << endl;
			msg.distance = (range_ + 1.0 - PARKING_DISTANCE) * 1000.;
			msg.speed = 900.;
			moving_ = false;
			parking_ = true;
			//lastPushing_ = true;
		...
		else
		{
			if (radarGoodData_) 
			{
				cout << " we are in parking position, shut it down at a distance  = " << range_ << " meters" << endl;
				return AllDoneState_;
			}
			else
			{
...
void setupStates()
{
   ...
   RadarApproachState_= fsm_.add_state("RadarApproachState");
   ...
   fsm_.set_entry_function(RadarApproachState_, boost::bind(&on_enter_RadarApproachState));
   fsm_.set_update_function(RadarApproachState_, boost::bind(&on_update_RadarApproachState));
   //fsm_.set_exit_function(RadarApproachState_, boost::bind(&on_exit_RadarApproachState));
