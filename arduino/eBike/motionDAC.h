...
public:
    RobotBase()
    {
      ...
      previousLeftDirection_ = 0; // only used in gooseMotors()
      previousRightDirection_ = 0;
      
     
              
      pidLinear_ = new pidControl(KP_LINEAR, KI_LINEAR, KD_LINEAR, MAX_PID_LINVEL_CHANGE_PER_CYCLE, MAX_LINEAR_ROBOT_VELOCITY);      
    }
  ...
  */
    ...
    void startAutoMove(float distance, float angle, float suggestedSpeed)  // distance in mm, angle in degrees, speed in mm/sec or deg/sec
    {
      ...
      if (fabs(angle) < 1.)  // this is a linear move 
      {
        ...
        DEBUG_SERIAL_PORT.print("starting linear autoMove, distance = ");
        DEBUG_SERIAL_PORT.println(distance);
      ...
      else // this is an angular move
      {
        ...
        linearCommandedVelocity_ = 0;
        DEBUG_SERIAL_PORT.print("starting angular autoMove, angle = ");
        DEBUG_SERIAL_PORT.println(angle);
    ...
    */
    ...
    {      
      ...
      */
        ...
        if (autoMoveMode_)
        {
          ...
          if (autoMoveStopped_) // just waiting to stop rolling
          {
            ...
            { 
              ...
              {
                DEBUG_SERIAL_PORT.print("autoMove completed, initial distance, final distance, time taken (ms), average velocity (m/s) , total move (mm) = ");
                ...
                DEBUG_SERIAL_PORT.print(", ");
                DEBUG_SERIAL_PORT.print(distanceMoved / (millis() - autoMoveStartTime_));
                DEBUG_SERIAL_PORT.print(", ");
               ...
               {                  
                DEBUG_SERIAL_PORT.print("autoMove turn done: initial yaw, final yaw, time taken (ms), avg vel (deg/sec), total turn = ");
                DEBUG_SERIAL_PORT.print(autoInitialYaw_);
                DEBUG_SERIAL_PORT.print(", ");
                DEBUG_SERIAL_PORT.print(gyro_stabilizer.getYawTotal());
                DEBUG_SERIAL_PORT.print(", ");
        ...
        if (fabs(linearCommandedVelocity_) < 50.) linearTargetVelocity = 0;  // for pure angular moves, just assign linear velocity to 0
          ...
          || ((linearCommandedVelocity_ + angCommandedVelocityMMperSec < -50) && rightSpeed_ > DAC_LOWER_VALUE) )
        {
          //if (DEBUG) 
          {
            ...
            DEBUG_SERIAL_PORT.print("current linear, angular (deg/sec) velocities = ");          
            DEBUG_SERIAL_PORT.print(linearCurrentVelocity_);
            DEBUG_SERIAL_PORT.print(", ");
            DEBUG_SERIAL_PORT.println(angCurrentVelocity_);
          }
          if ( ((linearCommandedVelocity_ - angCommandedVelocityMMperSec > 50) && leftSpeed_ < -DAC_LOWER_VALUE )
          || ((linearCommandedVelocity_ - angCommandedVelocityMMperSec < -50) && leftSpeed_ > DAC_LOWER_VALUE )) 
          {
...
private:
    ...
    unsigned long autoMoveStartTime_;
