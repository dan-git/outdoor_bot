    ...
    virtual void loop() {
      if (radio_control_allowed) {       
        ...
        {        
          ...
          if  ( ((right_speed * previousRightRadioDirection_ < 0 ) && fabs(right_speed) > 0) 
             ||  ((left_speed * previousLeftRadioDirection_ < 0 ) && fabs(left_speed) > 0 ) )
          {
            DEBUG_SERIAL_PORT.print("radio changed direction, now going left, right = ");
            DEBUG_SERIAL_PORT.print(previousLeftRadioDirection_);
            DEBUG_SERIAL_PORT.print(", ");
            DEBUG_SERIAL_PORT.println(previousRightRadioDirection_);
            int direction = rcs::sign(right_speed);
